...
public abstract class CcBinary implements RuleConfiguredTargetFactory {
  ...
  public static void init(
    ...
    ImmutableList<Artifact> debugFileOutputs = ccLinkingOutputsBinary.getDebugFiles();
    // If debug files (a la PDB) are generated by the link action, we add an output group
    if (debugFileOutputs != null && debugFileOutputs.size() > 0) {
      NestedSet<Artifact> debugFilesSet = new NestedSetBuilder(Order.STABLE_ORDER).addAll(debugFileOutputs).build();
      ruleBuilder.addOutputGroup("debug_files", debugFilesSet);

      if (featureConfiguration.isEnabled(CppRuleClasses.GENERATE_PDB_FILE) && debugFileOutputs.size() == 1) {
        ruleBuilder.addOutputGroup("pdb_file", debugFilesSet);
      }
    }
  ...
  public static Pair<CcLinkingOutputs, CcLauncherInfo> createTransitiveLinkingActions(
    ...
    CcLinkingHelper ccLinkingHelper =
        new CcLinkingHelper(
            ...
            .emitPdbFile(featureConfiguration.isEnabled(CppRuleClasses.GENERATE_PDB_FILE));
